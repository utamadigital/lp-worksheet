...
function planTitle(id: PlanId) {
  return id === "basic"
    ? "Basic - 1.000 halaman"
    : "Bundle - 1.500 halaman";
}

function planDesc(id: PlanId) {
  return id === "basic"
    ? "Untuk mulai rutinitas belajar Islami harian di rumah (tanpa bingung cari ide)."
    : "Untuk yang ingin lebih banyak variasi: total 1.500 halaman (lebih worth it untuk pemakaian jangka panjang).";
}

function planFeatures(id: PlanId) {
  return id === "basic"
    ? [
        "Total 1.000 halaman (PDF siap cetak A4)",
        "Hijaiyah + harakat + doa harian",
        "Wudhu + adab (kebiasaan baik)",
        "Akses instan + print ulang seumur hidup",
      ]
    : [
        "Total 1.500 halaman (lebih banyak variasi aktivitas)",
        "Lebih worth it: +500 halaman vs Basic",
    ...
    >
      <div className="flex items-start justify-between gap-3">
        ...
        <button
          ...
          className="inline-flex h-10 shrink-0 items-center justify-center rounded-xl border border-sky-200 bg-white px-3 text-xs font-extrabold text-sky-800 shadow-sm hover:bg-sky-50 focus:outline-none focus:ring-2 focus:ring-sky-200"
        >
          Detail
        </button>
      </div>

      {/* Modal daftar file - mobile safe (scroll & close) */}
      <BottomSheetModal
        open={show}
        onClose={() => setShow(false)}
        title="Daftar file paket Basic"
        subtitle="Total: 1.000+ lembar • PDF siap cetak A4"
      ...
      >
        <div className="rounded-2xl border border-sky-200 bg-sky-50 p-4">
          ...
          <p className="mt-3 text-[11px] text-slate-500">
            Semua file berbentuk PDF dan siap dicetak ulang kapan saja.
          </p>
        </div>
      </BottomSheetModal>
    </div>
  ...
  return (
    ...
    >
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2">
        ...
        {/* RIGHT: Summary */}
        <div id="checkout-summary" className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm md:sticky md:top-24 md:self-start">
          <div className="flex items-start justify-between gap-3">
            <div>
